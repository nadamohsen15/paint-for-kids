...
void ActionAddHexagon::Execute()
{
	...
	int length = abs(topLeft.x - bottomright.x) / 2;

	if ((topLeft.x - length) < 0 || (bottomright.x + length) > UI.width) {
		pGUI->PrintMessage("the shape exceeds the drawing area");
		return;
	}
	/*if ((bottomright.x + length) > UI.width) {
		pGUI->PrintMessage("the shape exceeds the drawing area");
		return;
	}*/
